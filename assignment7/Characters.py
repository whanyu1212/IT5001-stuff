...
class ArchMage(Mage):
    ...
    def count_alive(self, myTeam):
        count = 0
        for i in range(len(myTeam)):
            if myTeam[i].name != 'ArchMage' and myTeam[i].alive:
                count += 1
        return count
    
    def cast(self,myTeam,enemy):
        self.mana -= manaCost
        if self.count_alive(myTeam) == 0:
            dprint('Cast KABOOOOOOM ! (Damage 800) to every enemy!')
            # ! need to check if all their hp == 0
